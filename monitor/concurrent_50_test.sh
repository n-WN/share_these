...
BINARY_PATH="$PROJECT_DIR/target/release/share_these"
TEST_DIR="$PROJECT_DIR"
RESULTS_DIR="$SCRIPT_DIR/concurrent_50_test_$(date +%Y%m%d_%H%M%S)"
...
echo "🎯 面试场景：50用户同时并发测试（黑洞模式 + 修正本地回环重复计算）"
echo "================================================================"
...
echo "1️⃣ 启动文件服务器..."
cd "$TEST_DIR"
nohup "$BINARY_PATH" --port $SERVER_PORT > "$RESULTS_DIR/server.log" 2>&1 &
SERVER_PID=$!
    ...
    while true; do
        if [ -f "$stats_file" ]; then
            ...
            if [ -n "$latest" ] && [[ "$latest" != *"时间,内存"* ]]; then
                ...
                echo "💡 按 Ctrl+C 停止监控 | 💾 黑洞模式 + 📈 EWMA智能平滑算法"
...
echo ""
echo "3️⃣ 预热测试..."
if curl -s -o /dev/null -w "%{http_code}" http://localhost:$SERVER_PORT/files/$TEST_FILE | grep -q "200"; then
    echo "✅ 预热成功"
else
    echo "❌ 预热失败"
    exit 1
...
for i in $(seq 1 $CONCURRENT_USERS); do
    {
        ...
        url="http://localhost:$SERVER_PORT/files/$TEST_FILE"
